...
export function ProjectList() {
  ...
  const { data: projects, loading, error, execute: fetchProjects } = useApi<Project[]>(apiService.getProjects, { onSuccess: () => fetchProjects() });
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewProject({
        clientName: '',
        clientEmail: '',
        building: { type: '', size: 0, floors: 1, zones: 1 },
        status: 'draft',
      });
      fetchProjects(); // Refetch projects after creation
    } catch (error) {
  ...
  if (loading && !projects) { // Show loader only on initial load
    return (
              ...
              <div>
                ...
                <div className="relative rounded-md shadow-sm">
                  ...
                  <input
                    type="number"
                    id="buildingSize"
                    value={newProject.building.size}
                    onChange={(e) => setNewProject((prev) => ({ ...prev, building: { ...prev.building, size: Number(e.target.value) } }))}
                    className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md py-2.5"
                    placeholder="50000"
                    required
                  />
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-xl font-bold text-blue-700 truncate">{project.clientName}</h3>
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${project.status === 'completed'
                        ? 'bg-green-100 text-green-700'
                        : project.status === 'in_progress'
                          ? 'bg-yellow-100 text-yellow-700'
                          : project.status === 'draft'
                            ? 'bg-gray-100 text-gray-700'
                            : 'bg-red-100 text-red-700' // for rejected or other statuses
                      }`}
                  >
